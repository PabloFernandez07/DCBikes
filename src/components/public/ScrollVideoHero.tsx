...
function Reveal({
  ...
  progress: number;
  range: { inStart: number; inEnd: number; outStart: number; outEnd: number };
  staticMode: boolean;
  children: ReactNode;
  style?: CSSProperties;
}) {
...
export function ScrollVideoHero({ onQuoteOpen }: ScrollVideoHeroProps) {
  ...
  const { progress, isMobile, isReducedMotion } = useScrollVideo(sectionRef, videoRef);

  // En móvil o reduced-motion: vídeo en autoplay-loop, sección 100dvh, texto estático.
  const lock = !isMobile && !isReducedMotion;
  const scrollIndicatorOpacity = lock
    ? progress < 0.04 ? 1 : Math.max(0, 1 - (progress - 0.04) / 0.04)
    : 1;
      ...
      >
        ...
        <div className="absolute inset-0 z-10 flex items-center px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-7 max-w-xl lg:max-w-2xl">
            {/* Badge */}
            <Reveal progress={progress} range={RANGES.badge} staticMode={!lock}>
            ...
            {/* Título */}
            <Reveal progress={progress} range={RANGES.title} staticMode={!lock}>
              <h1
            ...
            <Reveal progress={progress} range={RANGES.divider} staticMode={!lock}>
            ...
            {/* Párrafo */}
            <Reveal progress={progress} range={RANGES.paragraph} staticMode={!lock}>
              <p
            ...
            {/* CTAs */}
            <Reveal progress={progress} range={RANGES.ctas} staticMode={!lock}>
